...
class SemiBothAugBaseRecognizer(BaseRecognizer):
    """Base class for semi-supervised recognizers.

    Strongly-augment both labeled and unlabeled data.
    ...
    def forward(self, imgs, label=None, return_loss=True, imgs_weak=None, imgs_strong=None, label_unlabeled=None, human_mask=None, imagenet_prob=None, human_mask_labeled=None):
        ...
        if return_loss:
            ...
            return self.forward_train(imgs, label, imgs_weak, imgs_strong, label_unlabeled, human_mask, imagenet_prob, human_mask_labeled)
        else:
    ...
    def train_step(self, data_batch_labeled, data_batch_unlabeled, optimizer, **kwargs):
        ...
        _, imgs, human_mask_labeled = self._actor_cutmix(data_batch_labeled)
        imgs_weak, imgs_strong, human_mask = self._actor_cutmix(data_batch_unlabeled)
        ...
        label = data_batch_labeled['label_unlabeled']
        label_unlabeled = data_batch_unlabeled['label_unlabeled']

        del data_batch_labeled['imgs_weak']

        losses = self(imgs, label, imgs_weak=imgs_weak, imgs_strong=imgs_strong, label_unlabeled=label_unlabeled, human_mask=human_mask, imagenet_prob=imagenet_prob, human_mask_labeled=human_mask_labeled)
    ...
    def _actor_cutmix(self, data_batch):
        ...
        return imgs_weak, imgs_strong, human_mask
